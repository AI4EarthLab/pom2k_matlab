...
bond=ff(im,:,:);
for k=1:kbm1
    ff(:,:,k)=DXF2_XY( AXB1_XY(dy).*( AXB1_XY(dt).*AXB1_XY(f(:,:,k)).*u(:,:,k)-AXB1_XY(aam(:,:,k)).*AXB1_XY(h)*tprni.*DXB1_XY(fb(:,:,k)).*dum.*DIVISION( 1.0,AXB1_XY(dx) ) ) )...
              +DYF2_XY( AYB1_XY(dx).*( AYB1_XY(dt).*AYB1_XY(f(:,:,k)).*v(:,:,k)-AYB1_XY(aam(:,:,k)).*AYB1_XY(h)*tprni.*DYB1_XY(fb(:,:,k)).*dvm.*DIVISION( 1.0,AYB1_XY(dy) ) ) )...
              +temp(:,:,k)/dz(k);
    ff(:,:,k)=( (fb(:,:,k)+fclim(:,:,k) ).*( OP_L_XY*(h+etb)*OP_R_XY ).*art-dti2*ff(:,:,k))./((h+etf).*art);
end
ff(im,:,:)=bond;
return