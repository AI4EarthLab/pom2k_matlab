...
%
      for k=1:kbm1
        for j=2:jmm1
          for i=2:im
            if(ff(i,j,k)<value_min || ff(i-1,j,k)<value_min)
              xmassflux(i,j,k)=0.e0;
            else
              udx=abs(xmassflux(i,j,k));
              u2dt=dti2*xmassflux(i,j,k)*xmassflux(i,j,k)*2.e0 ./(aru(i,j)*(dt(i-1,j)+dt(i,j)));
      ...
      for k=1:kbm1
        for j=2:jm
          for i=2:imm1
            if(ff(i,j,k)<value_min|| ff(i,j-1,k)<value_min) 
              ymassflux(i,j,k)=0.e0;
            else
             vdy=abs(ymassflux(i,j,k));
             v2dt=dti2*ymassflux(i,j,k)*ymassflux(i,j,k)*2.e0 /(arv(i,j)*(dt(i,j-1)+dt(i,j)));
...
%
      for k=2:kbm1
        for j=2:jmm1
          for i=2:imm1
            if(ff(i,j,k)<value_min|| ff(i,j,k-1)<value_min)
              zwflux(i,j,k)=0.e0;
            else
