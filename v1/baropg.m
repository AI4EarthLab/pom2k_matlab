...
% *                                                                    *
% **********************************************************************
global zz_3d grav dum_3d dvm_3d dz_3d im jm kb aru_3d arv_3d

rho = rho - rmean;
conmat=ones(im,jm,kb); conmat(:,:,1)=0.e0;
...
drhox=ramp * grav * AXB(dt_3d) .* SUMZ(-DZB(zz_3d).* AXB(dt_3d) .* DXB(AZB(rho)) + AZB(zz_3d) .* DXB(dt_3d).* DZB(AXB(rho)).*AZB(dz_3d).*conmat).* dum_3d;
...
drhoy=ramp * grav .* AYB(dt_3d) .* SUMZ(-DZB(zz_3d) .* AYB(dt_3d) .* DYB(AZB(rho))+ AZB(zz_3d) .* DYB(dt_3d) .* DZB(AYB(rho)).*AZB(dz_3d).*conmat).* dvm_3d;
%
rho = rho + rmean;