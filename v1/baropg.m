 function [drhox,drhoy] = baropg(rho, rmean, dt_3d, ramp)
% **********************************************************************
% *                                                                    *
...
drhoy=ramp * grav .* AYB(dt_3d) .* SUMZ(-DZB(zz_3d) .* AYB(dt_3d) .* DYB(AZB(rho-rmean))+ AZB(zz_3d) .* DYB(dt_3d) .* DZB(AYB(rho-rmean)).*AZB(dz_3d)).* dvm_3d;
%
% rho = rho + rmean;