...
clear all;
global et nwad;
% global pdens      % used by dens &profq
% global wriv       % used by advt
...
% lyo:%wad:Set wadsmoth>0 to (cosmetically) smooth out isolated,
...
cwetrlx1=(1.-dtrat)/(1.+dtrat);
cwetrlx2=2.*dtrat/(1.+dtrat);
alpha=alph0*(1-nwad) ; % lyo:!wad: must=0 for WAD
hhi=hhi0*(nwad);
nsmolar=nsmolar*nwad;

% local var
wadsmoth=0. ; % lyo:!wad:
cflmin=0.   ; % lyo:_20080415:

wriv =zeros(im,jm);
dwet =zeros(im,jm);wetmask =zeros(im,jm);wmarsh=zeros(im,jm);
ddf  =zeros(im,jm);dd0     =zeros(im,jm);ddb   =zeros(im,jm);
hkeep=zeros(im,jm);ssurfwet=zeros(im,jm); % used for river specifications



pdens=zeros(im,jm,kb) ;             % used in dens & profq



if( (nsmolar~=0)&&(mode~=2) )
	fprintf(' Stopped; incorrect inputs of\n');
	fprintf(' nsmolar   = %d\n', nsmolar);
	fprintf(' mode      = %d\n', mode   );
	fprintf(' For nsmolar ne 0, the present version works  \n') ;
	fprintf(' only if mode = 2. So either run in barotropic \n');
	fprintf(' mode = 2, or set nsmolar=0 and resubmit \n');
	error('');
end
% WADE
...
if(iproblem == 1)
	...
	uabw,uabe,ele,elw,tbe,tbw,sbe,sbw,tbn,tbs,sbn,sbs] = seamount(dx,dy,cor,pdens,hhi, ...
	east_c,north_c,east_e,north_e,east_u,north_u,east_v,north_v,...
	h,art,aru,arv,fsm,dum,dvm,...
	tsurf,ssurf,tb,sb,tclim,sclim,ub,uab,elb,etb,dt,...
	aam2d,rho,rmean,rfe,rfw,rfn,rfs,...
...
elseif(iproblem == 2)
	...
	tsurf,ssurf,tb,sb,tclim,sclim,ub,uab,elb,etb,dt,...
	aam2d,rho,rmean,rfe,rfw,rfn,rfs,...
	uabw,uabe,ele,elw,tbe,tbw,sbe,sbw,tbn,tbs,sbn,sbs,rot,tatm,satm,vfluxf,tbias,sbias] = box(dx,dy,cor,pdens,hhi,...
	east_c,north_c,east_e,north_e,east_u,north_u,east_v,north_v,...
	h,art,aru,arv,fsm,dum,dvm,...
	tsurf,ssurf,tb,sb,tclim,sclim,ub,uab,elb,etb,dt,...
	aam2d,rho,rmean,rfe,rfw,rfn,rfs,...
...
cort=cor(fix(im/2),fix(jm/2));
if (cort~=0.0)
	period=(2.*pi)/abs(cort)/86400.;
else
...
for j=1:jm
	for i=1:im
		...
		% vers.2:  cbc(i,j)=(kappa/log((zsh+(zz(kbm1)-z(kb))*d(i,j))/z0b))^2)  !
		% lyo:!wad:lyo's originals:---------------------------------------------!
...
for j=1:jm
	for i=1:im
		...
		if (fsm(i,j)~=0.0)
			cflmin=min(cflmin,tps(i,j)); % lyo:_20080415:
		end
	end
end
d = h+el;
...
for  iint=1:iend
	...
	for j=2:jmm1
		for i=2:imm1
			...
			%     Set w(i,j,1)=vflux(i,j)~=0 if one wishes non-zero flow across
			...
			%     constant; if also vflux(i,j)~=0, then, for example, the average
	...
	%     "if(mode==2)" loop in subroutine advave.
	...
	for iext=1:isplit    % Begin external (2-D) mode
		...
		% WAD by zhaowei 2017/11/9
		if (nsmolar==1 && mode==2)   !lyo:_20080415:
			dd0=h+ el*fsm;
			ddb=h+elb*fsm;
			tps(:,:)=0.0;           % aam2d(:,:)
			[ddf]=wadadvt2d(ddb,dd0,ddf,nitera,sw,dx,dy,ua,va,art,aru,arv,    ...
		...
		if (nwad==1)
			for j=1:jm;
				for i=2:im
					if (0.5*(dwet(i,j)+dwet(i-1,j))<hc)
						uaf(i,j)=0.0;
					else                      % Set flux=0 if coming from dry cell:
						if (wetmask(i-1,j)==0.0 && uaf(i,j)>0.0)
							uaf(i,j)=0.0;
						end
						if (wetmask(i,j)  ==0.0 && uaf(i,j)<0.0)
							uaf(i,j)=0.0;
						end
					end
				end
			...
			for j=2:jm;
				for i=1:im
					if (0.5*(dwet(i,j)+dwet(i,j-1))<=hc)
						vaf(i,j)=0.0;
					else                      % Set flux=0 if coming from dry cell:
						if (wetmask(i,j-1)==0.0 && vaf(i,j)>0.0)
							vaf(i,j)=0.0;
						end
						if (wetmask(i,j)  ==0.0 && vaf(i,j)<0.0)
							vaf(i,j)=0.0;
						end
					end
				end;
	...
	if(vamax<=vmaxl)
		...
		if((iint~= 1|| time0~=0.e0)&&mode~=2)
			...
			[ee,gg,l,kq,km,kh,...
			uf,vf,q2b,q2lb]=profq(ee,gg,l,kq,km,kh,pdens,zsh,...
			uf,vf,q2,q2b,q2lb, ...
			...
			if(mode~=4)
				...
				for j=1:jm
					for i=1:im
						ssurfwet(i,j)=ssurf(i,j)*(1.-wriv(i,j)/( wriv(i,j)+1.e-28)  );
					end
				end
				if(nadv==1)
				...
				if (nwad==1)
					for k=1:kb-1;
						for j=1:jm;
							for i=1:im
								if (wetmask(i,j)==0.0)
									uf(i,j,k)=(cwetrlx1*tb(i,j,k)+cwetrlx2*tsurf(i,j)*fsm(i,j));
									vf(i,j,k)=(cwetrlx1*sb(i,j,k)+cwetrlx2*ssurfwet(i,j)*fsm(i,j));
								end
							end;
						end;
					end
			...
			if (nwad==1)
				for k=1:kb-1
					for j=1:jm;
						for i=2:im
							if (wetmask(i,j)*wetmask(i-1,j)==.0)
								uf(i,j,k)=0.0;
							end
						end
					end
					for j=2:jm;
						for i=1:im
							if (wetmask(i,j)*wetmask(i,j-1)==0.0)
								vf(i,j,k)=0.0;
							end
						end
					end
	...
	if(mod(iint,iprint)==0 || vamax>=vmaxl)
		...
		for k=1:kbm1
			for j=1:jm
				for i=1:im
					darea=dx(i,j)*dy(i,j)*wetmask(i,j); % WAD Zhaowei 2017/11/9
