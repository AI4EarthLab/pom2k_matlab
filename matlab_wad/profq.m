function [ee,gg,l,kq,km,kh,...
	...
	q2lb=abs(q2lb);
	for k=2:kbm1
		boygr(:,:,k)=grav*(rho(:,:,k-1)-rho(:,:,k))./(dzz(k-1)* (h+et)) ...
		+(grav.^2) *2.e0./(cc(:,:,k-1).^2+cc(:,:,k).^2);      %add by hx
		% boygr(:,:,k)=grav*(rho(:,:,k-1)-rho(:,:,k))./( (h+et)) ;      %add by hx
	end
	%      for k=2:kbm1
	%        for j=1:jm
	%          for i=1:im
	...
	%                    /(dzz(k-1)* (h(i,j))+et(i,j))+(grav^2) ...
	%                    *2.e0/(cc(i,j,k-1)^2+cc(i,j,k)^2);      %add by hx
	%          end
	%        end
	%      end
	%
