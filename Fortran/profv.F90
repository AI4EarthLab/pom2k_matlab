subroutine profv(dti2)
  ...
  implicit none
  real(kind=8) :: dti2
  type(Matrix) ::  LAA, LBB, dh, dh_3d, cbc_3d, tps_3d, wvsurf_3d
  integer :: i, ierr
  !  a=zeros(im,jm,kb);
  ! dh = AYB(h+etf); dh(1,:)=1.e0; dh(:,1)=1.e0;
  dh = AYB(h + etf)
  call dm_setvalues(dh, (/0/), (/(i,i=0,jm-1)/), (/0/), (/(1, i=0, jm)/), ierr)
  call dm_setvalues(dh, (/(i,i=0,im-1)/), (/0/), (/0/), (/(1, i=0, im)/), ierr)
  ...
  !     a(:,:,kbm1)= 0.e0;
  d = -dti2 * shift(c+umol, 3, 1)
  a = d .ed. (dz_3d .em. dzz_3d .em. dh_3d .em. dh_3d)
  a = a .em. (1-shift(MASK_Z2, 3, 1))
  
  !     d(:,:,2:kbm1)=dzz_3d(:,:,1:kbm2);
  !     c=DIVISION(-dti2*(c+umol),dz_3d.*d.*dh_3d.*dh_3d);
  !     c(:,:,1)  =0.e0;
  d = (shift(dzz_3d, 3, 1) .em. MASK_Z2) + (d .em. (MASK_ZZ))
  c = (-dti2 * (c+umol)) .ed. (dz_3d .em. d .em. dh_3d .em. dh_3d)
  c = c .em. REV_MASK_Z1
