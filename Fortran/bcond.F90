subroutine new_bcond(idx, dti)
  ...
  real(kind=8), allocatable ::  arr_im(:), arr_jm(:)
  integer :: i, j
  ...
  if (idx == 1) then
     ...
     ! elf = elf .em. REV_MASK_X1 .em. REV_MASK_X2 + shift(elf, 1, 1) .em. MASK_X1 + shift(elf, 1, -1) .em. MASK_X2
     ! elf = elf .em. REV_MASK_Y1 .em. REV_MASK_Y2 + shift(elf, 2, 1) .em. MASK_Y1 + shift(elf, 2, -1) .em. MASK_Y2
     ! elf = elf .em. fsm
     ...
     tmpuvaf = dm_rep(uabe, im, 1, 1) + rfe * sqrt(grav * dm_pow(Li(h), -1)) .em. (Li(el) - dm_rep(ele, im, 1, 1))

     tmpuvaf = ramp * tmpuvaf

     call dm_getvalues(tmpuvaf, (/imm1/), (/(j, j=1,jmm2)/), (/0/), local_array1, ierr)
     call dm_setvalues(uaf, (/imm1/), (/(j, j=1,jmm2)/), (/0/), local_array1, ierr)

     tmpuvaf = dm_rep(uabw, im, 1, 1) - rfw * sqrt(grav * dm_pow(Li(h), -1)) .em. (Li(el) - dm_rep(elw, im, 1, 1))

     tmpuvaf = ramp * tmpuvaf

     call dm_getvalues(tmpuvaf, (/0/), (/(j, j=1,jmm2)/), (/0/), local_array1, ierr)
     call dm_setvalues(uaf, (/0/), (/(j, j=1,jmm2)/),(/0/), local_array1, ierr)
     call dm_setvalues(uaf, (/1/), (/(j, j=1,jmm2)/), (/0/), local_array1, ierr)

     local_array1 = dm_zeros(1, jmm2, 1)

     call dm_setvalues(vaf, (/imm1/), (/(j, j=1,jmm2)/), (/0/), local_array1, ierr)
     call dm_setvalues(vaf, (/0/), (/(j, j=1,jmm2)/),(/0/), local_array1, ierr)
     call dm_destroy(local_array1, ierr)

     tmpuvaf = dm_trans(dm_rep(vabn, jm, 1, 1)) + rfn * sqrt(grav * dm_pow(Rj(h), -1)) .em. (Rj(el) - &
          dm_trans(dm_rep(eln, jm, 1, 1)))

     tmpuvaf = ramp * tmpuvaf

     call dm_getvalues(tmpuvaf, (/(i, i=1,imm2)/), (/jmm1/), (/0/), local_array2, ierr)
     call dm_setvalues(vaf, (/(i, i=1,imm2)/), (/jmm1/),(/0/), local_array2, ierr)

     tmpuvaf = dm_trans(dm_rep(vabs, jm, 1, 1)) - rfs * sqrt(grav * dm_pow(Rj(h), -1)) .em. (Rj(el) - &
          dm_trans(dm_rep(els, jm, 1, 1)))

     tmpuvaf = ramp * tmpuvaf

     call dm_getvalues(tmpuvaf, (/(i, i=1,imm2)/), (/0/), (/0/),local_array2, ierr)
     call dm_setvalues(vaf, (/(i, i=1,imm2)/), (/0/),(/0/), local_array2, ierr)
     call dm_setvalues(vaf, (/(i, i=1,imm2)/), (/1/),(/0/),local_array2, ierr)

     local_array2 = dm_zeros(imm2, 1, 1)

     call dm_setvalues(uaf, (/(i, i=1,imm2)/), (/jmm1/),(/0/), local_array2, ierr)
     call dm_setvalues(uaf, (/(i, i=1,imm2)/), (/0/),(/0/), local_array2, ierr)
     call dm_destroy(local_array2, ierr)
     ...
     tmpuvf= sqrt((1.e0/hmax) * (h_3d .em. MASK_X2)) .em. (AXF1(AXB1(shift(u, 1, -1) .em. MASK_X2))) + &
          (1.e0 - sqrt((1.e0/hmax) * (h_3d .em. MASK_X2))) .em. (AXF1(AXB1(u .em. MASK_X2)))
     ...
     uf   = uf .em. REV_MASK_X2 + uf .em. MASK_X2 .em. MASK_Y1 + uf .em. MASK_X2 .em. MASK_Y2 + &
          uf .em. MASK_X2 .em. tmpMASK + tmpuvf .em. REV_MASK_Y1 .em. REV_MASK_Y2 .em. REV_MASK_Z2

     tmpuvf= sqrt((1.e0/hmax) * (h_3d .em. MASK_X1)) .em. (AXF1(AXB1(shift(shift(u, 1, 1), 1, 1) .em. MASK_X1))) +&
          (1.e0 - sqrt((1.e0/hmax) * (h_3d .em. MASK_X1))) .em. (AXF1(AXB1(shift(u, 1, 1) .em. MASK_X1)))

     uf   = uf .em. REV_MASK_X1 + uf .em. MASK_X1 .em. MASK_Y1 + uf .em. MASK_X1 .em. MASK_Y2 + &
          uf .em. MASK_X1 .em. tmpMASK + tmpuvf .em. REV_MASK_Y1 .em. REV_MASK_Y2 .em. REV_MASK_Z2

     uf   = uf .em. MASK_X1 + shift(shift(shift(shift(uf, 1, 1), 1, 1), 1, -1), 1, -1) + &
          shift((shift(uf, 1, 1) .em. MASK_X1 .em. MASK_Y1 + shift(uf, 1, 1) .em. MASK_X1 .em. MASK_Y2 + &
          shift(uf, 1, 1) .em. MASK_X1 .em. tmpMASK), 1, -1) + &
          shift((tmpuvf .em. REV_MASK_Y1 .em. REV_MASK_Y2 .em. REV_MASK_Z2), 1, -1)

     vf = vf .em. REV_MASK_X1 .em. REV_MASK_X2 + vf .em. (MASK_X1 + MASK_X2) .em. (MASK_Y1 + MASK_Y2) + &
          vf .em. (MASK_X1 + MASK_X2) .em. tmpMASK


     tmpuvf= sqrt((1.e0/hmax) * (h_3d .em. MASK_Y2)) .em. (AXF2(AXB2(shift(v, 1, -1) .em. MASK_Y2))) + &
          (1.e0 - sqrt((1.e0/hmax) * (h_3d .em. MASK_Y2))) .em. (AXF2(AXB2(v .em. MASK_Y2)))

     ! tmpMASK = MASK_Z2
     ! call dm_setvalue(tmpMASK, 1, jm, kb, 0.e0, ierr)
     ! call dm_setvalue(tmpMASK, im, jm, kb, 0.e0, ierr)

     vf   = vf .em. REV_MASK_Y2 + vf .em. MASK_Y2 .em. MASK_X1 + vf .em. MASK_Y2 .em. MASK_X2 + &
          vf .em. MASK_Y2 .em. tmpMASK + tmpuvf .em. REV_MASK_X1 .em. REV_MASK_X2 .em. REV_MASK_Z2

     tmpuvf= sqrt((1.e0/hmax) * (h_3d .em. MASK_Y1)) .em. (AXF2(AXB2(shift(shift(v, 2, 1), 2, 1) .em. MASK_Y1))) +&
          (1.e0 - sqrt((1.e0/hmax) * (h_3d .em. MASK_Y1))) .em. (AXF2(AXB2(shift(v, 2, 1) .em. MASK_Y1)))

     ! call dm_setvalue(tmpMASK, 1, 1, kb, 0.e0, ierr)
     ! call dm_setvalue(tmpMASK, im, 1, kb, 0.e0, ierr)

     vf   = vf .em. REV_MASK_Y1 + vf .em. MASK_Y1 .em. MASK_X1 + vf .em. MASK_Y1 .em. MASK_X2 + &
          vf .em. MASK_Y1 .em. tmpMASK + tmpuvf .em. REV_MASK_X1 .em. REV_MASK_X2 .em. REV_MASK_Z2

     vf   = vf .em. MASK_Y1 + shift(shift(shift(shift(vf, 2, 1), 2, 1), 2, -1), 2, -1) + &
          shift((shift(vf, 2, 1) .em. MASK_Y1 .em. MASK_X1 + shift(vf, 2, 1) .em. MASK_Y1 .em. MASK_X2 + &
          shift(vf, 2, 1) .em. MASK_Y1 .em. tmpMASK), 2, -1) + &
          shift((tmpuvf .em. REV_MASK_X1 .em. REV_MASK_X2 .em. REV_MASK_Z2), 2, -1)

     uf = uf .em. REV_MASK_Y1 .em. REV_MASK_Y2 + uf .em. (MASK_Y1 + MASK_Y2) .em. (MASK_X1 + MASK_X2) + &
     ...
     tmp1 = t .em. MASK_X2 - tmp .em. (dm_rep(tbe, im, 1, 1) .em. MASK_X2 - t .em. MASK_X2)
     tmp2 = s .em. MASK_X2 - tmp .em. (dm_rep(sbe, im, 1, 1) .em. MASK_X2 - s .em. MASK_X2)

     tmp3 = t .em. MASK_X2 - tmp .em. (t .em. MASK_X2 - shift(t, 1, -1) .em. MASK_X2)
     tmp4 = s .em. MASK_X2 - tmp .em. (s. em. MASK_X2 - shift(s, 1, -1) .em. MASK_X2)

     tmpwm= dti * AZF1(shift(w, 1, -1) .em. MASK_X2) .ed. (Rk1(zz_3d .em. MASK_X2) .em. (shift(dt_3d, 1, -1) .em. MASK_X2))
     ...
     uf = uf .em. REV_MASK_X2 + uf .em. MASK_X2 .em. MASK_Z2 + &
          (tmp1 .em. filter1 + (tmp3 .em. filter4 + tmp5 .em.filter3) .em. filter2) .em. REV_MASK_Z2
     vf = vf .em. REV_MASK_X2 + vf .em. MASK_X2 .em. MASK_Z2 + &
          (tmp2 .em. filter1 + (tmp4 .em. filter4 + tmp6 .em.filter3) .em. filter2) .em. REV_MASK_Z2
     ...
     tmp1 = t .em. MASK_X1 - tmp .em. (t .em. MASK_X1 - dm_rep(tbw, im, 1, 1) .em. MASK_X1)
     tmp2 = s .em. MASK_X1 - tmp .em. (s .em. MASK_X1 - dm_rep(sbw, im, 1, 1) .em. MASK_X1)

     tmp3 = t .em. MASK_X1 - tmp .em. (shift(t, 1, 1) .em. MASK_X1 - t .em. MASK_X1)
     tmp4 = s .em. MASK_X1 - tmp .em. (shift(s, 1, 1). em. MASK_X1 - s .em. MASK_X1)

     tmpwm= dti * AZF1(shift(w, 1, 1) .em. MASK_X1) .ed. (Rk1(zz_3d .em. MASK_X1) .em. (shift(dt_3d, 1, 1) .em. MASK_X1))
     ...
     tmp = (2.e0 * dti * (v .em. MASK_Y2)) .ed. (dy_3d .em. MASK_Y2 + shift(dy_3d, 2, -1) .em. MASK_Y2)
     ...
     tmp1 = t .em. MASK_Y2 - tmp .em. (dm_rep(tbn, 1, jm, 1) .em. MASK_Y2 - t .em. MASK_Y2)
     tmp2 = s .em. MASK_Y2 - tmp .em. (dm_rep(sbn, 1, jm, 1) .em. MASK_Y2 - s .em. MASK_Y2)

     tmp3 = t .em. MASK_Y2 - tmp .em. (t .em. MASK_Y2 - shift(t, 2, -1) .em. MASK_Y2)
     tmp4 = s .em. MASK_Y2 - tmp .em. (s. em. MASK_Y2 - shift(s, 2, -1) .em. MASK_Y2)

     tmpwm= dti * AZF(shift(w, 2, -1) .em. MASK_Y2) .ed. (Rk2(zz_3d .em. MASK_Y2) .em. (shift(dt_3d, 2, -1) .em. MASK_Y2))
     ...
     tmp = (2.e0 * dti * (shift(v, 2, 1) .em. MASK_Y1)) .ed. (dy_3d .em. MASK_Y1 + shift(dy_3d, 2, 1) .em. MASK_Y1)
     ...
     tmp1 = t .em. MASK_Y1 - tmp .em. (t .em. MASK_Y1 - dm_rep(tbs, 1, jm, 1) .em. MASK_Y1)
     tmp2 = s .em. MASK_Y1 - tmp .em. (s .em. MASK_Y1 - dm_rep(sbs, 1, jm, 1) .em. MASK_Y1)

     tmp3 = t .em. MASK_Y1 - tmp .em. (shift(t, 2, 1) .em. MASK_Y1 - t .em. MASK_Y1)
     tmp4 = s .em. MASK_Y1 - tmp .em. (shift(s, 2, 1). em. MASK_Y1 - s .em. MASK_Y1)

     tmpwm= dti * AZF(shift(w, 2, 1) .em. MASK_Y1) .ed. (Rk2(zz_3d .em. MASK_Y1) .em. (shift(dt_3d, 2, 1) .em. MASK_Y1))
